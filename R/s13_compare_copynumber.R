...
if (SWITCH) {
	...
	g_cn = foreach (i=1:nrow(tracker)) %dopar% {
		...
		load(path, envir=data)
		seg = data$CN %>%
			  dplyr::select(chrom=Chromosome, start = Position, log2 = Log2Ratio) %>%
			  mutate(end = start) %>%
			  filter(chrom<23)
...
if (SWITCH) {
	...
	data = foreach (i=1:nrow(tracker)) %dopar% {
		...
		if (tracker$GRAIL_ID[i]=="MSK-VB-0012") {
			x1 = .15
		}
		if (tracker$GRAIL_ID[i]=="MSK-VP-0019") {
			x1 = .15
		}
...
for (i in 1:length(cols)) {
	rect(xleft=z+.23, xright=z+.23+.2, ybottom=i-.5, ytop=i+.5, col=cols[i], border="white", lwd=.1)
}

...
tmp = clinical %>%
	  ...
	  mutate(cat = "") %>%
	  mutate(cat = ifelse(patient_id %in% rownames(data_ch)[index_control], "Control", cat)) %>%
	  mutate(cat = ifelse(patient_id %in% rownames(data_ch)[index_late], "Cancer", cat)) %>%
	  mutate(cat_num = 0) %>%
	  mutate(cat_num = ifelse(patient_id %in% rownames(data_ch)[index_control], 2, cat_num)) %>%
	  mutate(cat_num = ifelse(patient_id %in% rownames(data_ch)[index_late], 1, cat_num)) %>%
...
plot.0 = ggplot(tmp.0, aes(x=cat, y=median_age)) +
  		 geom_bar(stat="identity", fill="steelblue", color="black") +
 		 geom_point(alpha = .8, size = 2.5, color = "black", fill = "steelblue", shape = 21, aes(x=jitter(cat_num), y=age), data=tmp) +
...
pdf(file="../res/rebuttal/Age_by_Cancer.pdf", width=3)
print(plot.0)
dev.off()
